...
//						 rs.getInt(7),rs.getDouble(8),1));
	...
	public int editPass(String tenDN,String password) {
		String sql = "Update TAI_KHOAN set MAT_KHAU=? where TEN_TAI_KHOAN='"+tenDN+"'";
...
//	}
	...
	public void themDanhGia(String maKH, String maSp,String danhGia,int danhgiaSao) {
		...
		String sql ="insert into DANH_GIA(MA_KHACH_HANG,MA_SP,DANH_GIA,DIEM_DANH_GIA) values  (?,?,?,?)";
		try {
			conn = new DBContext().getConnection();
			ps = conn.prepareStatement(sql);
			ps.setString(1, maKH);
